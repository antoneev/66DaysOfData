...
def unknown(cleanedWords):
    ...
    for word in cleanedWords:
        result = database.searchForWords(word)
        if result == False:
            count = extractText.syllable_count(word)
            if count > 2:
...
def main(check):
    if check == 'PDF':
        textFromPDF = extractText.pdfToText()
        cleanedWords = extractText.wordCleaningPDF(textFromPDF)
        ...
        cleanedWords = extractText.wordCleaningText(text)
        searchForWords = unknown(cleanedWords)
        print(searchForWords)
